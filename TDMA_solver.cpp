...
static double uint2double_ript(uint64_dt in){
	data_conv tmp;
	tmp.i = in;
	return tmp.f;
}

static uint64_dt double2uint_ript(double in){
	data_conv tmp;
	tmp.f = in;
	return tmp.i;
}
...
static void read_coeff(const uint512_dt*d, hls::stream<uint256_dt> &d_stm,
	...
	switch(dirXYZ){
		case 0 : {d0 = (M>>3); d1 = N; d2 = L; off_d0 = 1; off_d1 = (M>>3); off_d2 = N*(M>>3); break;}
		case 1 : {d0 = (M>>3); d1 = N; d2 = L; off_d0 = 1; off_d1 = (M>>3); off_d2 = N*(M>>3); break;}
		case 2 : {d0 = (M>>3); d1 = L; d2 = N; off_d0 = 1; off_d1 = N*(M>>3); off_d2 = (M>>3); break;}
		default :{d0 = (M>>3); d1 = N; d2 = L; off_d0 = 1; off_d1 = (M>>3); off_d2 = N*(M>>3); break;}
	}
	...
	ap_uint<10> batd = 0;
		...
		for(int itr= 0; itr < total_itr; itr++){
			...
			int ind = bat*(M>>3)*N*L + j*off_d1 + i *off_d2;
...
//	printf("read_coeff didn't got struck\n");
...
static void stream_8x8transpose(hls::stream<uint256_dt> &in, hls::stream<uint256_dt> &out0,
	...
	unsigned int total_blocks = ((M >> 2)*((N*L*B+3)>>2));
	unsigned int total_read_c = ((M >> 2)*((N*L*B+3)>>2)<<2);

//	bool re_order = (dirXYZ == 0);
...
//	printf("stream_8x8transpose didn't got struck\n");
}
...
static void thomas_interleave(hls::stream<uint256_dt> &d_stm, hls::stream<uint256_dt> &d_fw_stm,
	...
	loop_read: for(int itr= 0; itr < total_itr; itr++){
			...
			ap_uint<24> countr1 = register_it<int>((bat<<6) + i);
...
static void thomas_forward(hls::stream<uint256_dt> &d_fw_stm, hls::stream<uint256_dt> &c2_fw_stm, hls::stream<uint256_dt> &d2_fw_stm,
	...
	loop_fw: for(int itr= 0; itr < total_itr; itr++){
		...
		fw_vec_loop: for(int v =0; v < VEC_FACTOR; v++){
			#pragma HLS unroll
			double aa_read = (i == 0 || i == d0 -1) ? 0.0 : -0.5; // uint2double_ript(a2_read.range(D_SIZE*(v+1)-1,D_SIZE*v));
			double bb_read = (i == 0 || i == d0 -1) ? 1.0 :  2.0; //uint2double_ript(b2_read.range(D_SIZE*(v+1)-1,D_SIZE*v));
			double cc_read = (i == 0 || i == d0 -1) ? 0.0 : -0.5; //uint2double_ript(c2_read.range(D_SIZE*(v+1)-1,D_SIZE*v));
			double dd_read = uint2double_ript(d2_read.range(D_SIZE*(v+1)-1,D_SIZE*v));

			double bb_old = uint2double_ript(vec_bb_r.range(D_SIZE*(v+1)-1,D_SIZE*v));
			double dd_old = uint2double_ript(vec_dd_r.range(D_SIZE*(v+1)-1,D_SIZE*v));
			double cc_old = uint2double_ript(vec_cc_r.range(D_SIZE*(v+1)-1,D_SIZE*v));



			double denom = (i == 0) ? bb_read : (bb_read - aa_read*cc_old);
			double r = 1.0/denom;
			double c_w1 = cc_read;
			double d_w1 = (i == 0) ? dd_read : (dd_read - aa_read*dd_old);
			...
			b2_fw_write.range(D_SIZE*(v+1)-1,D_SIZE*v) = double2uint_ript(b_wr);
			d2_fw_write.range(D_SIZE*(v+1)-1,D_SIZE*v) = double2uint_ript(d_wr);

			vec_bb_w.range(D_SIZE*(v+1)-1,D_SIZE*v) = double2uint_ript(b_wr);
			vec_dd_w.range(D_SIZE*(v+1)-1,D_SIZE*v) = double2uint_ript(d_wr);
			vec_cc_w.range(D_SIZE*(v+1)-1,D_SIZE*v) = double2uint_ript(c_wr);
...
static void thomas_backward(hls::stream<uint256_dt> &c2_fw_stm, hls::stream<uint256_dt> &d2_fw_stm, hls::stream<uint256_dt> &u_stm,
	...
	loop_bw: for(int itr= 0; itr < total_itr; itr++){
		...
		bw_vec_loop: for(int v = 0; v < VEC_FACTOR; v++){
			#pragma HLS unroll
			double dd_read = uint2double_ript(d2_fw_read.range(D_SIZE*(v+1)-1,D_SIZE*v));
			double cc_read = uint2double_ript(c2_fw_read.range(D_SIZE*(v+1)-1,D_SIZE*v));

			double u_pre = uint2double_ript(vec_u2_r.range(D_SIZE*(v+1)-1,D_SIZE*v));
			double numer_l = dd_read;
			double numer_o = (dd_read - cc_read * u_pre);
			double numer = (i == d0-1) ? numer_l : numer_o;

			double u_new = numer;
			u2_write.range(D_SIZE*(v+1)-1,D_SIZE*v) = double2uint_ript(u_new);
			vec_u2_w.range(D_SIZE*(v+1)-1,D_SIZE*v) = double2uint_ript(u_new);
		}
...
//	printf("write_u didn't got struck\n");
...
//
//sp=TDMA_batch_4.d_1:HBM[12:13]
//sp=TDMA_batch_4.u_1:HBM[12:13]
//sp=TDMA_batch_4.acc1_1:HBM[12:13]
//sp=TDMA_batch_4.acc2_1:HBM[12:13]
//
//sp=TDMA_batch_4.d_2:HBM[14:15]
//sp=TDMA_batch_4.u_2:HBM[14:15]
//sp=TDMA_batch_4.acc1_2:HBM[14:15]
//sp=TDMA_batch_4.acc2_2:HBM[14:15]
//
//sp=TDMA_batch_5.d_1:HBM[16:17]
//sp=TDMA_batch_5.u_1:HBM[16:17]
//sp=TDMA_batch_5.acc1_1:HBM[16:17]
//sp=TDMA_batch_5.acc2_1:HBM[16:17]
//
//sp=TDMA_batch_5.d_2:HBM[18:19]
//sp=TDMA_batch_5.u_2:HBM[18:19]
//sp=TDMA_batch_5.acc1_2:HBM[18:19]
//sp=TDMA_batch_5.acc2_2:HBM[18:19]
//
//sp=TDMA_batch_6.d_1:HBM[20:21]
//sp=TDMA_batch_6.u_1:HBM[20:21]
//sp=TDMA_batch_6.acc1_1:HBM[20:21]
//sp=TDMA_batch_6.acc2_1:HBM[20:21]
//
//sp=TDMA_batch_6.d_2:HBM[22:23]
//sp=TDMA_batch_6.u_2:HBM[22:23]
//sp=TDMA_batch_6.acc1_2:HBM[22:23]
//sp=TDMA_batch_6.acc2_2:HBM[22:23]


//
//slr=TDMA_batch_4:SLR1
//slr=TDMA_batch_5:SLR2
//slr=TDMA_batch_6:SLR2
